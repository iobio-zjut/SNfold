...
extern int stage3_flag;
extern int stage4_flag;
extern int sequence_lenth;
extern Real score_max;
extern Real score_pose_maximum;
extern Real last_accepted_dscore;
extern Real lowest_dscore;
extern core::pose::Pose pose_lowest_dscore;
extern core::pose::Pose pose_maximum;
extern utility::vector1< int > RESIDUE1;
extern utility::vector1< int > RESIDUE2;
extern utility::vector1< Real > DISTANCE;
extern utility::vector1< Real > CONFIDENCE;
extern utility::vector1< Real > NICHE_RADIUS;
extern utility::vector1< Real > SCORE_MAX;
extern utility::vector1< core::pose::Pose > SEED_POSE;
extern utility::vector1< core::pose::Pose > MODAL_POSE;
///......................................................................
...
void ClassicAbinitio::apply( pose::Pose & pose ) {
	...
	//read distance
	ifstream distance_input("./input_files/alldist.txt");
	string distance_line;
	while ( getline( distance_input, distance_line ) ) {
		int residue1, residue2;
		Real distance, confidence;
		istringstream distance_data(distance_line);
		distance_data >> residue1 >> residue2 >> distance >> confidence;
		if ( residue2 > residue1 ) {
			RESIDUE1.push_back(residue1);
			RESIDUE2.push_back(residue2);
			DISTANCE.push_back(distance);
			CONFIDENCE.push_back(confidence);
		}
	}
...
///=============================modal exploration=============================
	...
	cout << "///////////////////////////////////////////////////////////////////" << std::endl;
	cout << "                         modal exploration" << std::endl;
	cout << "///////////////////////////////////////////////////////////////////" << std::endl;
	
	exploit_flag = 0;
	serial_number = 0;
	
	stage1_cycles_ = static_cast< int > (2000);
	stage2_cycles_ = static_cast< int > (2000);
	stage3_cycles_ = static_cast< int > (2000);
	stage4_cycles_ = static_cast< int > (4000);
	
	while ( serial_number < trajectory ) {
		cout << '\n';
		cout << "---------------------------------" << serial_number + 1 << "---------------------------------" << std::endl;
		
		pose = init_pose;
		score_max = -10000;
		score_pose_maximum = -10000;
		...
		//add the max score
		SCORE_MAX.push_back(score_max);

		//calculate niche radius
		niche_radius = angle_distance( pose, pose_maximum );
		NICHE_RADIUS.push_back(niche_radius);
...
///=============================modal exploitation=============================
	...
	cout << "///////////////////////////////////////////////////////////////////" << std::endl;
	cout << "                         modal exploitation" << std::endl;
	cout << "///////////////////////////////////////////////////////////////////" << std::endl;
	...
	stage4_cycles_ = static_cast< int > (40000);
	
	while ( serial_number < trajectory ) {
		cout << '\n';
		cout << "---------------------------------" << serial_number + 1 << "---------------------------------" << std::endl;

		pose = SEED_POSE[serial_number+1];
		last_accepted_dscore = distance_function( pose );
		lowest_dscore = last_accepted_dscore;
		pose_lowest_dscore = pose;
		
		//MMC simulation
		run_stage1( pose );
		pose = pose_lowest_dscore;
		run_stage2( pose );
		pose = pose_lowest_dscore;
		run_stage3( pose );
		pose = pose_lowest_dscore;
	...
	for ( int i = 1; i <= trajectory - 1; ++i ) {
		for ( int j = i + 1; j <= trajectory; ++j ) {
			if ( ( distance_function( MODAL_POSE[j] ) < distance_function( MODAL_POSE[i] ) ) || ( distance_function( MODAL_POSE[j] ) == distance_function( MODAL_POSE[i] ) && (*SCORE3)( MODAL_POSE[j] ) < (*SCORE3)( MODAL_POSE[i] ) ) ) {
				temp_pose = MODAL_POSE[i];
				MODAL_POSE[i] = MODAL_POSE[j];
				MODAL_POSE[j] = temp_pose;
			}
		}
	}
	...
	for ( int i = 1; i <= 5; ++i ) {
		...
		pose_name << "./output_files/model_" << i << ".pdb";
		MODAL_POSE[i].dump_pdb(pose_name.str());
	}
...
Real ClassicAbinitio::modified_fitness_function( core::pose::Pose pose, Real init_score ) {
...
//derating function
Real ClassicAbinitio::derating_function( core::pose::Pose pose1, core::pose::Pose pose2, Real r ) {
	Real distance;
...
Real ClassicAbinitio::angle_distance( core::pose::Pose pose1, core::pose::Pose pose2 ) {
	Real sum = 0;
	Real distance;
...
Real ClassicAbinitio::distance_function( core::pose::Pose pose ) {
